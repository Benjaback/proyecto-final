...
const buttonClasses = {
    ...
    reopen: "bg-yellow-500 hover:bg-yellow-600 text-gray-900"
};

const TerminalModal = ({ show, onConfirm, onCancel, data, type }) => { 
    if (!show || !data) {
        return null;
    }
    const isDelete = type === 'delete';
    const title = isDelete ? 'Confirmar Eliminación' : 'Confirmar Rehacer';
    const message = isDelete 
        ? 'Estás a punto de ELIMINAR esta tarea permanentemente. ¿Deseas continuar?' 
        : '¿Deseas REAHCER esta tarea ?';
    const confirmText = isDelete ? 'Sí, Eliminar' : 'Si, Rehacer';
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-transparent">
            ...
            <div className="bg-gray-800 rounded-xl shadow-2xl p-6 w-11/12 max-w-sm border border-indigo-500/50 transform transition-all duration-300 scale-100 z-50">
                ...
                <div className="flex justify-center space-x-3">
                    ...
                    <button
                        onClick={onConfirm}
                        className={`py-2 px-4 font-bold rounded-lg transition duration-200 ${type === 'reopen' ? buttonClasses.reopen : buttonClasses[type]}`}
                    >
                        {confirmText}
                    </button>
                </div>
